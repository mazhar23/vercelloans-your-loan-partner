  ...
  head: () => ({
    meta: [
      { title: "Apply — VercelLoans" },
      {
        name: "description",
        content:
          "Apply for a VercelLoans personal loan in minutes. Soft credit check, no impact to your score.",
      },
    ],
  }),
  component: ApplyPage,
    ...
    return (
      <div className="container mx-auto px-4 py-20 max-w-2xl text-center">
        ...
        <p className="mt-3 text-muted-foreground">
          Thanks, {data.fullName?.split(" ")[0]}! We're reviewing your application and will email{" "}
          <strong>{data.email}</strong> with your personalized offers within minutes.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="container mx-auto px-4 py-12 max-w-3xl">
      ...
      <div className="flex items-center justify-between mb-8">
        {steps.map((s, i) => (
          <div key={s} className="flex-1 flex items-center">
            <div
              className={`flex flex-col items-center ${i <= step ? "text-foreground" : "text-muted-foreground"}`}
            >
              <div
                className={`h-8 w-8 rounded-full grid place-items-center text-xs font-semibold ${i <= step ? "text-primary-foreground" : "bg-muted"}`}
      ...
      <div className="rounded-2xl border border-border bg-card p-6 md:p-8 shadow-sm">
        {step === 0 && (
          <div className="space-y-5">
            ...
            <Field label="Loan amount" error={errors.loanAmount}>
              ...
              >
                {["1000", "2500", "5000", "7500", "10000", "15000", "20000", "25000", "35000"].map(
                  (a) => (
        ...
        {step === 4 && (
          <div className="space-y-5">
            ...
            <dl className="rounded-xl bg-secondary/50 p-5 text-sm grid grid-cols-2 gap-y-2">
              ...
              <dd>${Number(data.loanAmount).toLocaleString()}</dd>
              <dt className="text-muted-foreground">Purpose</dt>
              <dd>{data.loanPurpose}</dd>
              ...
              <dd>{data.employmentStatus}</dd>
              <dt className="text-muted-foreground">Income</dt>
              <dd>${Number(data.annualIncome || 0).toLocaleString()}</dd>
            </dl>
            <label className="flex items-start gap-3 text-sm">
              ...
              />
              <span>
                I authorize VercelLoans to perform a soft credit inquiry and contact me about loan
                offers. I have read the <span className="underline">Privacy Policy</span> and{" "}
                <span className="underline">E-sign Consent</span>.
              </span>
            </label>
        ...
        <div className="mt-8 flex items-center justify-between gap-3">
          <button
            onClick={back}
            disabled={step === 0}
            className="rounded-full border border-border bg-card px-5 py-2 text-sm font-semibold disabled:opacity-50 hover:bg-muted"
          >
            Back
          </button>
          {step < steps.length - 1 ? (
      ...
      <p className="mt-6 text-center text-xs text-muted-foreground">
        We never ask for your online banking password, SSN, or full account credentials on this
        form.
      </p>
    </div>
  );
